...
var Signup = function (_Form) {
	...
	function Signup() {
		...
		_this.state.el = document.getElementById('stSignupForm');
		...
		_this.get('/signup/init', function (data) {
			...
			data.html.forEach(function (ele, i, a) {
				if (ele.length === 0) {
					return _this.state.html[i] = ele;
				}
				...
				return _this.state.html[i] = temp.content.firstChild;
			});
			_this.step(function () {
				_this.overlay();
			});
	...
	_createClass(Signup, [{
		...
		value: function next(action) {
			...
			if (action === 'void') return this.step(function () {
				_this2.overlay();
			});

			if (!this.state.customer[action].submitted) this.post('/signup/' + action, this.state.customer[action], function (d) {
				if (d.code === 'signup_error') return _this2.printError(d.message) && _this2.overlay();

				_this2.state.customer[action].submitted = true;
				_this2.step(function () {
					_this2.overlay();
				});
			});
		}
...
var Login = function (_Form) {
  ...
  _createClass(Login, [{
    ...
    value: function submit() {
      ...
      this.post('/auth/token', function (d) {
